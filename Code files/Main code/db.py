...
for row in results:
    ...
    if (row[0] not in ("celebrity_tweets","celebrities",'celebrity_mentioned_count','celebrity_distinct_hashtags_count','celebrity_hashtags_count','users_common','hashtags_common','celebrity_common_mention_tweets')):
...
for index in range(len(friends_ids)):
    ...
    for index1 in range(index+1,len(friends_ids)):
        ...
        for row3 in results3:
            common_users.write("\nUsers Common Between "+row3[0]+" and "+row3[1]+"\n")
        
        query_check2 = """ select count(Distinct concat(c.user_twitter_id,' ',c.tweet)) from celebrity_tweets as c JOIN celebrity_tweets as c1 ON c.users_mention_id = """+friends_ids[index]+""" and c1.users_mention_id = """+friends_ids[index1]+""" where ( (concat_ws(' ',c.user_twitter_id,c.tweet)) in (concat_ws(' ',c1.user_twitter_id,c1.tweet)) ) """      
        ...
        for row2 in results2:
            ...
            if row2[0] == 0:
                ...
                common_users.write("\n"+str(row2[0])+"\n")
                common_users.write("\n*****************************\n")                
                query_check4 = """ insert into celebrity_common_mention_tweets(celebrity1_name,celebrity2_name,celebrity1_id,celebrity2_id,common_mention_tweets) values('%s','%s','%s','%s','%d')""" % (row3[0],row3[1],friends_ids[index],friends_ids[index1],row2[0])
